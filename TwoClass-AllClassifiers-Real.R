...
for (u in 1 : iterations) {
   ...
   data.train <- DodgerLoopGame.train %>% na.omit %>% as.matrix
   data.test <- DodgerLoopGame.test %>% na.omit %>% as.matrix
   ...
   # labels.train <- unique(data.train[,1])
   # data.train.c1 <- data.train[which(data.train[,1] == labels.train[1]), ]
   # data.train.c2 <- data.train[which(data.train[,1] == labels.train[2]), ]
