...
for(u in 1:ITER){
   ...
   Tjj <- lapply(data.training.list.unlab, function(df){
      ...
      tcos <- dissim.cos(train.set = as.matrix(df), no.cores = no.cores)
      tcos.comp <- dissim.cos.comp(train.set = as.matrix(df), no.cores = no.cores)
   ...
   colnames(Tjj) <- c('cos', 'cos.comp', 'sin', 'sin.comp')
   ...
   lbl.ensmbl <- t(parApply(cl, data.test[,-1] , 1, function(Z){
      ...
      return(c(which.min(Tjj$cos/2 - TjZ[,1]), 
               which.min(Tjj$cos.comp/2 - TjZ[,1]),
               which.min(Tjj$sin/2 - TjZ[,1]),
...
colnames(out2) = c('delta0.cos', 'delta0.cos.comp',
