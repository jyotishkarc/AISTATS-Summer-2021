...
for (k in 1:length(d.seq)) {
   ...
   result <- foreach(u = 1:iterations,
        ...
        mdl.bayes.Y <- apply(test.sample, 1, function(val){
           return(sum(log(dpareto(val, location = 2, shape = 1))))
        })
...
res.list.Par11.vs.Par21 <- list("d=5" = res.list[[1]],
                                "d=10" = res.list[[2]],
                                "d=25" = res.list[[3]],
                                "d=50" = res.list[[4]],
                                "d=100" = res.list[[5]],
...
# writexl::write_xlsx(x = res.list.Par11.vs.Par21,
#                     path = "E:\\Jyotishka\\Code\\Pop-Par11-vs-Par21-with-nn.xlsx")
