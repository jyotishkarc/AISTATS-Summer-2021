...
for (k in 1:length(d.seq)) {
   ...
   result <- foreach(u = 1:iterations,
        ...
        mdl.bayes.X <- apply((test.sample+1), 1, function(val){
           return(sum(dexp(val, 1, log = T)))
        })
        ...
        res.list[[k]] <- rbind(result, rep(NA, 18), apply(result, 2, mean), 
                               apply(result, 2, sciplot::se))
   }

res.list.exp.location.change <- list("d=5" = res.list[[1]],
                                  "d=10" = res.list[[2]],
                                  "d=25" = res.list[[3]],
                                  "d=50" = res.list[[4]],
                                  "d=100" = res.list[[5]],
...
writexl::write_xlsx(x = res.list.exp.location.change,
                    path = "E:\\Jyotishka\\Code\\Pop-exp-location-change-with-nn.xlsx")
