   ...
   result <- foreach(u = 1:iterations,
                     ...
                     .packages = c('glmnet','RandPro','e1071','randomForest',
                                   'class','nnet')) %dopar% {
                                      ...
                                      mdl.bayes.Y <- apply((test.sample+0.5), 1, function(val){
                                         return(sum(dexp(val, 2, log = T)))
                                      })
...
res.list.exp.scale.change <- list("d=5" = res.list[[1]],
                            "d=10" = res.list[[2]],
                            "d=25" = res.list[[3]],
                            "d=50" = res.list[[4]],
                            "d=100" = res.list[[5]],
...
writexl::write_xlsx(x = res.list.exp.scale.change,
                    path = "E:\\Jyotishka\\Code\\Pop-exp-scale-change-with-nn.xlsx")
