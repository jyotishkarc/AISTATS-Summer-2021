...
df.exp.location.change <- rio::import_list("~/R/R Codes/Classification of HDLSS Data (Summer, 2021)/HDLSS-Summer-2021/TwoClass-PopularClassifiers-Simulated-JRC/Results/Pop-exp-location-change-with-nn.xlsx")
...
for (k in 1:length(d.seq)) {
   ...
   df.exp.location.change[[k]]$SVMRBF <- c(result, NA, mean(result), 
                                           sciplot::se(result))
...
writexl::write_xlsx(x = df.exp.location.change,
                    "C:\\Users\\JYOTISHKA\\Desktop\\Pop-exp-location-change-updated.xlsx")
