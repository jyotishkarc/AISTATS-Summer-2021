...
time.matrix.multi <- matrix(0, nrow = length(d.seq), ncol = 16)
...
for (k in 1:length(d.seq)) {
   ...
   system.time({
      ...
      T.sin <- matrix(0, no.of.classes, no.of.classes)
      ...
      for (i in 1 : no.of.classes) {
         for (j in 1:i) {
            ...
            y1 <- dissim.sin(rbind(mat1,mat2), no.cores = no.cores) %>% as.matrix()
      ...
      lbl.ensmbl <- t(apply(test.sample , 1, function(Z){
         Z <- as.numeric(Z)
         ...
         indicator_Z.sin <- ind.Z.sin <- matrix(0, no.of.classes, no.of.classes)
   ...
   system.time({
      ...
      p_1 <- as.numeric(predict(fit1, test.sample %>% as.matrix()))
   ...
   system.time({
      ...
      p_1 <- as.numeric(predict(fit4, test.sample %>% as.matrix()))
   ...
   # system.time({
   #    mdl.bayes.X <- apply(test.sample %>% as.matrix(), 1, function(val){
   #    return(sum(dnorm(val, 1, 1, log = T)))
   #    })
   # 
   #    mdl.bayes.Y <- apply(test.sample %>% as.matrix(), 1, function(val){
   ...
   time.matrix.multi[k, ] <- c(time.delta0.sin[3], 
...
time.matrix.multi <- time.matrix.multi %>% as.data.frame()

rownames(time.matrix.multi) <- as.character(d.seq)
colnames(time.matrix.multi) <- c('Delta-0','Delta-1','Delta-2',
                                 'GLMNET',
                                 'ONN',
                                 'NNRAND',
...
# write_xlsx(cbind("Dimensions" = as.character(d.seq), time.matrix.multi),
#       "C:\\Users\\JYOTISHKA\\Desktop\\Time-Matrix-All-Classifiers-Multi-Observation.xlsx"
#       )

write_xlsx(cbind("Dimensions" = as.character(d.seq), time.matrix.multi),
           "E:\\Jyotishka\\Code\\Time-Matrix-All-Classifiers-Multi-Observation.xlsx"
           )
